<template>
  <div class="p-2">
    <el-card class="mb-2">
      ...
      <pre
        class="font-mono whitespace-pre-wrap break-words bg-gray-100 p-2.5 rounded text-gray-700"
        >{{ mcpServerConfig }}</pre
      >
    </el-card>
    ...
    <el-row :gutter="8">
      ...
      >
        <el-card class="mb-5 min-h-[150px] flex flex-col overflow-hidden">
          ...
          >
            ...
            <div class="space-y-2">
              ...
              >
                ...
                <div
                  class="text-gray-500 mt-0.5 text-xs line-clamp-2"
          ...
          <div
            v-else
            class="mt-1 text-xs p-2 border-t border-gray-200 bg-gray-50 rounded-b flex items-center justify-center"
          >
    ...
    >
      ...
      >
        <el-form-item
          v-for="(propDetails, propName) in currentTestingTool.inputSchema
            .properties"
          :key="propName"
          :label="propDetails.description || propName"
          :prop="propName"
          :rules="formRules(propName, propDetails)"
        >
          <el-input
          ...
          >
            <el-option
              ...
              :key="enumValue"
              :label="getEnumLabel(currentTestingTool.name, enumValue)"
              :value="enumValue"
            />
          </el-select>
          <el-input
            ...
            v-model="testParamsForm[propName]"
            :placeholder="
              (propDetails.description || propName) + ' (请输入JSON格式)'
            "
      ...
      <div
        ...
        class="mt-5 p-[15px] border border-gray-200 rounded bg-gray-50"
      >
        <h4 class="mt-0 mb-2.5 text-base">API 返回结果:</h4>
        <div v-if="typeof apiDialogResponse === 'string'">
          <pre
            class="bg-gray-100 p-2.5 rounded whitespace-pre-wrap break-words overflow-y-auto"
            >{{ apiDialogResponse }}</pre
          >
        </div>
        <div
          v-else-if="
            apiDialogResponse.type === 'image' && apiDialogResponse.content
          "
        >
          <el-image
        ...
        <div
          v-else-if="
            apiDialogResponse.type === 'text' && apiDialogResponse.content
          "
        >
          <pre
            class="bg-gray-100 p-2.5 rounded whitespace-pre-wrap break-words overflow-y-auto"
...
<script setup>
    ...
    if (testParamsFormRef.value) {
      testParamsFormRef.value.validate(async (valid) => {
        if (valid) {
          ...
          if (
            currentTestingTool.value.inputSchema &&
            currentTestingTool.value.inputSchema.properties
          ) {
            Object.keys(
              currentTestingTool.value.inputSchema.properties
            ).forEach((propName) => {
              const propDetails =
                currentTestingTool.value.inputSchema.properties[propName]
              if (
                (propDetails.type === 'object' ||
                  propDetails.type === 'array') &&
                payload[propName] &&
                typeof payload[propName] === 'string'
              ) {
                try {
                  payload[propName] = JSON.parse(payload[propName])
                } catch (e) {
                  ElMessage.error(
                    `参数 ${propName} 的JSON格式无效: ${e.message}`
                  )
  ...
  const formRules = (propName, propDetails) =>
    currentTestingTool.value.inputSchema.required &&
    currentTestingTool.value.inputSchema.required.includes(propName)
      ? [
          {
            required: true,
  ...
  const getEnumLabel = (toolName, enumValue) => {
    if (toolName === 'currentTime') {
      return timezoneMap[enumValue]
        ? `${enumValue}-${timezoneMap[enumValue]}`
        : enumValue
    }
    return enumValue
  }
